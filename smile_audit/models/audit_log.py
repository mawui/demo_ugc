...
class AuditLog(models.Model):
    ...
    model_id = fields.Many2one(
        'ir.model', 'Model ID', required=True, readonly=True)
    model = fields.Char(
    ...
    def _get_name(self):
        for log in self:
            if log.model_id and log.res_id:
                record = self.env[log.model_id.model].browse(
                    log.res_id).exists()
                if record:
                    log.name = record.display_name
                else:
                    data = safe_eval(log.data or '{}', {'datetime': datetime})
                    rec_name = self.env[log.model_id.model]._rec_name
                    if rec_name in data['new']:
                        log.name = data['new'][rec_name]
                    elif rec_name in data['old']:
                        log.name = data['old'][rec_name]
                    else:
                        log.name = 'id=%s' % log.res_id
            else:
                log.name = ''
